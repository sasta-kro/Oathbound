...
|---|---|---:|---|
| `content/creatures/` | `CreatureSpecies` | 26 | Types, base stats, growth curve, learnset, evolution, ability pool, bind chance, sprites. |
| `content/moves/` | `MoveData` | 21 | Power, accuracy, cooldown, priority, status, stat modifiers, VFX preset. |
| `content/abilities/` | `AbilityData` | 10 | Declarative passives (Spec 9.9). |
| `content/quests/` | `QuestData` + `QuestObjective` | 5 | Giver actor id, `requires`, objectives, rewards, eight dialogue lines. |
| `content/growth/` | `GrowthCurve` | 2 | XP and stat curves. |
| `content/types/` | `TypeChart` + `TypeMatchup` | 1 | Only non-neutral matchups listed. |
| `content/vfx/` | `VfxPreset` | 18 | Shape, colours, timing for the one effect shader. |
| `content/sprites/` | `SpriteFrames` | 28 | Player hero, knight, 26 creature battle sheets. Monster-pack sheets, including element recolours (`PALETTES`), are generated by `tools/import_monster_sprites.py`. |
...
- Bosses are `WildCreature`s with a `boss_id` (plus `required_quest`, `challenge_line`, `sealed_line`, `victory_line`). `main._challenge_boss` asks before fighting, `BattleConfig.boss()` forbids running and binding, a victory calls `GameState.record_boss_defeat` (level cap from `GameState.BOSS_LEVEL_CAPS`), and `_wire_area` hides bosses already beaten.
...
Save keys today: `saved_at`, `play_seconds`, `party[]` (species, level, total_xp, current_hp, moves[], ability), `seen_species[]`, `binding_scrolls`, `currency`, `level_cap`, `defeated_bosses[]`, `quests{}`, `location{area,x,y,facing_x,facing_y}`, `format_version`.
...
| `test_boss_battle.gd` | Boss config rules, level cap raise, persistence, quest-gated challenge, Area One placement. |
