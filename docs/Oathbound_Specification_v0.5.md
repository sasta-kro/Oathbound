...
- Status tags describe the code as of the document date: **[Implemented]** means the section is in the game and covered by tests where practical; **[Partial]** means part of it exists, with the gap named; **[Not started]** means no code exists. Tags are informational and never change a rule's authority.
...
**[Partial]** Two areas exist: `areas/town.tscn` and `areas/area_one.tscn`, linked both ways through the north gate. The town is at once Hub 1 and the Area 1 starter settlement: it holds the healer (the Knight), the main-quest giver (the Elder) and two side-quest givers. Whether a separate hub is ever needed is open; the current plan is that services grow inside the town. Areas 2 and 3 do not exist. The legacy `areas/test_01.tscn` is a test room and not part of the world.
...
- The player can interact with NPCs and world objects that are within reach. Because movement is analog, "adjacent" means within a radius of **1.5 cells** (Provisional) of the player from any side, not a neighbouring grid cell.
- The player does not personally fight in combat. The lead Oathbound walks at the player's heel and throws the overworld strike (Section 7.5).
...
**[Implemented]** except the player name, which is not asked for or stored yet. Controls are listed in Section 22.1.
...
**[Not started]** No interactable objects beyond NPCs and creatures. Signs are painted decor without text.
...
**[Not started]** Walls, water and props block movement; nothing else.
...
- Move toward the player after detection, giving up when the player is 1.5x the detection radius away or the chase has taken the creature more than 2 cells beyond its leash.
- On reaching striking range (1.5 cells) the creature stops and **winds up** for a short telegraph (Provisional **0.45 s**), then strikes. The strike only lands if the player is still within reach, so stepping back dodges it. A landed strike starts a battle on the creature's terms (Section 7.5).
- After striking, or after being hit, the creature is open for a moment and cannot re-target.

Exact detection distances, movement speeds and the windup are Provisional balance values.

**[Implemented]** `scripts/wild_creature.gd`. Placeholder: a creature with no art shows a labelled box, per Section 23.
...
A creature the player ran from is held off for a short grace period (Provisional 1.5 s) so fleeing is not instantly undone. **[Implemented]**

### 7.5 Overworld Strike and Battle Opening

The first blow of an encounter is struck in the overworld, not on the battle screen. This is the game's main addition to the creature-collecting formula and it is **MVP**.

**Player strike.** Pressing the strike key makes the lead Oathbound (the first non-fainted party member, walking beside the player) lunge at the nearest visible creature inside a wedge in front of the player. Provisional values: reach **2.4 cells** centre to centre, wedge **150 degrees**, anything within **1.25 cells** is hit regardless of facing, **0.45 s** cooldown between swings. The lunge always plays so a miss reads as a miss.

**What the strike does.** The lead uses whichever of its damaging moves would hurt this creature most, and lands exactly the damage that move would deal in battle (Section 11.8), with abilities and type applied. A creature that knows no damaging move still lands 1 damage. Then:

- If the creature survives, a battle starts with the creature already at its reduced HP and the player's lead taking the first action of turn one ("advantage").
- If the creature drops to 0 HP, it is **routed** where it stands. No battle opens. The player receives the same XP and currency the battle would have paid, and forfeits the chance to bind it.

**Creature strike (ambush).** A hostile creature that lands its own windup strike hits the lead for **0.6x** (Provisional) of its best move, never taking the lead below 1 HP. The battle then opens with the creature acting first on turn one ("disadvantage"). The player has no way to answer an ambush before the battle screen opens, which is why it is softer and cannot knock the lead out.

**Persistence of overworld damage.** Damage dealt in the overworld stays on the creature: the battle opens with it, and it is still there if the player runs and comes back, until the creature is defeated or respawned. Creatures that have been hurt show a small HP bar.

**Interaction with the rest of the rules.** Turn one order is forced by the opening (Section 11.3); from turn two the normal priority and Speed rules apply. A party with no usable Oathbound cannot strike; walking into a creature still opens a battle and the party is revived on the way in (Provisional, until the defeat and revival rules of Section 20 are complete).

**[Implemented]** `scripts/player.gd`, `scripts/world/overworld_partner.gd`, `scripts/world/overworld_strike.gd`, `scripts/main.gd`. Covered by `tests/test_overworld_strike.gd`.

## 8. Hostile Oathkeeper Encounters

**[Not started]** No hostile Oathkeeper exists in the overworld. The battle engine already supports multi-creature enemy parties, non-escapable battles and a trainer name (`BattleConfig.trainer()`), so the missing pieces are the overworld NPC, its detection, and defeat persistence.
...
**[Implemented]** capacity and duplicates. There is no way to remove a party member yet, so the final-member rule has nothing to guard.
...
**[Not started]** No Hotel. With a full party the Bind command is disabled with the reason "Your party is full", so no scroll is wasted.
...
**[Partial, policy open]** In battle, participants only. Two other sources of XP currently pay **every non-fainted party member**: an overworld rout (Section 7.5) and quest XP rewards. Whole-party payout for quest rewards is intended. For routs it is Provisional and inconsistent with the battle rule; the recommended fix is to pay the lead only, since it threw the blow.

Provisional reward values: XP for defeating a creature is `round((BaseHP + BaseAttack + BaseDefense + BaseSpeed) x Level / 6)`, minimum 1.
...
**[Not started]** Over-cap XP is counted (`XpResult.excess`, `BattleEngine.xp_over_cap`) and discarded.
...
- Evolution immediately updates the creature's relevant species-derived stats and identity. Level, XP, moves and missing HP carry over; the ability carries over when the new species can have it (Section 9.9).
- The player is told when an Oathbound is ready to evolve and evolves it from the companion record in the field menu at any later time. Refusing is simply not pressing the button. No prompt interrupts a battle.

Branching evolution is Deferred.

**[Implemented]** Only Emberling evolves so far.
...
Each species defines a small **ability pool**, and every individual creature carries **exactly one** ability from that pool. Abilities are passive: they apply whenever the creature is the active battler and need no action.

#### 9.9.1 Selection

- A creature's ability is fixed when it is created. Wild creatures roll one at random from the pool per encounter; a hand-placed creature or a content script may pin one.
- A species may have an empty pool, in which case its creatures have no ability.
- The ability is saved with the creature. If content later removes that ability from the species pool, the creature falls back to the pool's first entry.

#### 9.9.2 Player Interaction

The player cannot change, reroll or remove an ability in the MVP. The ability and its description are shown on the Oathbound details screen.

#### 9.9.3 Evolution

On evolution the creature keeps the same ability if the new species can have it; otherwise it takes the ability in the same pool slot; otherwise the new species' first ability.

#### 9.9.4 Approved Effect Categories

An ability is declarative content. It may only combine these effects, each built on a mechanic the game already has:

- Passive stat modifiers (percentages, additive with Section 11.9 modifiers, same 0 to 200 percent clamp).
- A multiplier on damage dealt, optionally restricted to one move type.
- A multiplier on damage taken, optionally restricted to one move type.
- Immunity to specific status conditions.
- A bonus (or penalty) in percentage points to the chance of statuses the creature inflicts.

Introducing any other effect category is a specification decision, not a content change.

**[Implemented]** `scripts/creatures/ability_data.gd`; seven abilities in `content/abilities/` (Ember Body, Searing Touch, Stonewall, Swiftfoot, Thick Hide, Toxic Ward, Windrider).
...
**[Implemented]** Sections 10.1 to 10.4 as `content/types/type_chart_mvp.tres`; unlisted matchups resolve to 1.0x.
...
When every equipped move is on cooldown the creature **Waits**, doing nothing that turn. This applies to the enemy AI as well.
...
**Turn one exception.** When the encounter opened with an overworld strike (Section 7.5), the side that landed it acts first on turn one regardless of priority or Speed. Steps 3 to 5 apply from turn two.

**[Implemented]** `BattleEngine.take_turn`.
...
- A cooldown of N makes the move unavailable for exactly the next N turns after the turn it was used on.
...
**[Implemented]** Messages hold 0.55 s or advance on a key press; attacks present in 0.4 s; the stage is a side view (player left, enemy right and mirrored) because all creature art is drawn in profile.

### 11.13 Escaping

Running from a wild battle is a chance roll. Provisional formula:

`Chance = clamp(0.5 x RunnerSpeed / ChaserSpeed + 0.15 x PreviousAttempts, 0.10, 0.95)`

Run resolves before either creature's move (priority 7). A failed attempt lets the enemy act. **[Implemented]**
...
Provisional currency value: `10 + 5 x Level` per defeated creature. **[Implemented]** for wild battles and routs; trainers do not exist.
...
**[Partial]** The ordinary profile exists (`BattleAI`): expected damage after type and accuracy, else a status or buff the target lacks, ties broken randomly. No boss profile.
...
**[Partial]** NPCs have one idle line plus the eight quest lines (offer, accepted, refused, re-offer, progress, abandoned, complete, done). Choices are a two-option reply list. No sparring, shop or price-modifier dialogue yet.
...
**[Partial]** Steps 1, 2, 4 and 5 happen. There is no revival location yet, so recovery happens in place with a short line of dialogue, and the world is not reset.
...
This section was changed in v0.5 to match a decision taken during implementation. v0.4 asked for a single autosave slot and no manual save command.

- **Three manual save slots** the player writes on purpose from the field menu, plus **one autosave slot** the game writes at state boundaries.
- The autosave never overwrites a manual slot, and a manual save never touches the autosave.
- The title screen offers **Continue** (the most recently written slot of either kind), **Load a journey** (pick or erase any slot) and **Begin anew** (starts fresh without erasing anything).
- Loading mid-journey, overwriting a slot and erasing ask for a second confirmation.
- Each slot keeps its previous contents as a hidden backup, read when the slot itself fails to parse. Writes go to a temporary file first and are swapped into place, so a crash mid-write leaves the previous file intact.
- The save format carries a version number for future migrations.

**[Implemented]** `scripts/save/save_service.gd`, `user://saves/`.
...
Current bindings (Provisional until rebinding exists):
...
Menus accept the mouse and standard keyboard focus traversal.
...
**[Implemented]** Logical viewport 960x540, window sizes offered as whole multiples that fit the screen, borderless fullscreen so the window stays minimisable, settings persisted.
...
**[Implemented]** including status badges next to the types, the player's XP bar, floating damage numbers and a bind-chance hint on the Bind command.
...
- The innate ability and its description.

**[Implemented]** Companion record also lets a healthy companion become party lead and evolves a ready one.
...
**[Partial]** The field journal lists every species with UNSEEN / SEEN / BOUND, searchable by name or element. Seen state is saved. **Known gap:** BOUND reflects the current party only; "ever bound" is not recorded, so it should become a persisted set next to `seen_species`.
...
| Content | MVP Target | Current (12 Sep 2026) |
|---|---:|---:|
...
- Do not modify this specification unless a developer explicitly requests a specification change. When a change is made, bump the version, archive the previous file under `docs/archive/`, and record what changed in the header.
...
- Overworld strike reach, wedge, cooldown, windup and ambush multiplier.
- Starter species and level.
- XP policy for overworld routs (lead only vs. whole party).
- Whether the town remains the only hub.
...
**Playable today:** steps 1 to 4 and 6, in the town and Area One, with healing at the Knight and two main plus two side quests. Steps 5 (shops, Hotel, moves) and 7 to 11 are open.
...
- **Large Binary Assets:** **Git LFS.** Large binary source assets are kept out of normal Git object history where appropriate. *Not yet configured:* there is no `.gitattributes`, and every sprite sheet so far is small enough that plain Git has been fine. Set it up before large audio or animation drops arrive, or drop this line.
...
- **Formatting and Linting:** **gdtoolkit 4.x.** `gdformat` and `gdlint` provide consistent GDScript formatting and automated static checks for both humans and coding agents. (pip install or brew install) *Not yet enforced:* several UI scripts are written in a compact style `gdformat` would rewrite. Run it once as its own commit.
...
- **CI:** **Headless Godot 4.7.2.** Continuous integration should run the same pinned engine version and automated validation used locally. *Not yet set up.*
