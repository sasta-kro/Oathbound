...
const QUEST_ID: StringName = &"quest_main_01e_no_battle_at_all"
...
## How far off it is put, in cells, on the side away from the Scout.
...
const SIGHTING: PackedStringArray = [
	...
	"Scout: See how little it has left? Walk over and hit it. Watch what does not happen.",
]
## Held on screen while the player closes. [code]%s[/code] is its name.
const PROMPT := "Finish the %s with F. It has too little left to give you a fight."
