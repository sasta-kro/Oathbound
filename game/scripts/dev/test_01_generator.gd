class_name TestOneGenerator
extends Node2D

## Paints the starting layout of the test map into its TileMapLayer children.
##
## Development only. `bake_test_01.gd` runs this once to produce the
## hand-editable `res://areas/test_01.tscn`; from then on the map is painted
...
## Atlas source ids, matching `res://assets/tilesets/overworld.tres`.
