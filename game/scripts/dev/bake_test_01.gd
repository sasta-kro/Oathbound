## Writes the sample map `res://areas/test_01.tscn`.
##
## Paints the generated test map layout into plain TileMapLayers, adds the
...
##
## Run with:
##   godot --headless --path game --script res://scripts/dev/bake_test_01.gd
extends SceneTree

const OUTPUT_PATH: String = "res://areas/test_01.tscn"
...
var _generator: TestOneGenerator
...
func _initialize() -> void:
	...
	_generator = TestOneGenerator.new()
...
func _bake() -> void:
	var tile_set: TileSet = load(TILESET_PATH)
	var generator: TestOneGenerator = _generator
	...
	for layer_name: String in ["Ground", "Path", "Walls", "Decor"]:
		...
		layer.name = layer_name
		layer.tile_set = tile_set
		layer.position = TestOneGenerator.ROOM_RECTANGLE.position
	...
	player_start.position = TestOneGenerator.cell_to_world(PLAYER_START_CELL)
	...
	knight.name = "Knight"
	knight.position = TestOneGenerator.cell_to_world(KNIGHT_CELL)
	...
	for definition: Dictionary in SPAWN_ZONES:
		...
		zone.position = TestOneGenerator.cell_to_world(definition["cell"])
